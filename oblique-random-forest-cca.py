...
class ObliqueDecisionTreeClassifier:
    ...
    def get_best_split(self, X, Y):
        ...
        for _ in range(self.n_projections):
            ...
            # Verifica se ha amostras/variacao suficiente antes de rodar o CCA.
            # Com poucas amostras (nos profundos da arvore) ou colunas quase
            # constantes, o ajuste do CCA fica degenerado (score com variancia
            # zero), o que gera divisao por zero (0/0) e pesos NaN.
            total_variance = np.var(X_subset, axis=0).sum()
            min_samples_for_cca = k + 2

            if X_subset.shape[0] >= min_samples_for_cca and total_variance > 1e-6:
                try:
                    with warnings.catch_warnings():
                        warnings.simplefilter("error", RuntimeWarning)
                        warnings.simplefilter("ignore", ConvergenceWarning)
                        cca = CCA(n_components=1, max_iter=1000)
                        cca.fit(X_subset, Y_target)
                        W_subset = cca.x_weights_[:, 0]
